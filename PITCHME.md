...
Note: Why might a checklist be useful? As a tool it compensates for the limits of mental capacity, and could help, for instance, a Material Sciences and Engineering researchers also be a bit of an information professional.
---
@title[Investment]
...
- Iterative process
Note: From the RDC's perspective, a checklist is attractive because we could quickly prototype a tool that, if successful, have an impact, and, if not, it would have been a fairly small investment of time. The process could also be easily iterated: we could roll something out, get feedback, improve, and repeat.
---
...
Note: We started with an environmental scan to make sure others hadn't already done this. We started by poking around the web, which was inefficient. Bertha ultimately sent a message to the DataCure list. We receive three responses. One was from the Data Stewardship Action Team (whose product was still in development and looked to require more technical build time than we were willing to invest); one was Johns Hopkins (which oriented to DMP reviews); one was from the National Center for Atmospheric Research (well-developed checklist, but specialized to them). But the responses were encouraging: it seemed like there was some room to develop and contribute a new tool.
---
@title[Audience]
...
Note: We figured the checklist ought to follow the research data lifecycle.
---
@title[Research Lifecycle]
### Data Lifecycle
<img src="assets/research-lifecycle.png" alt="Research Lifecycle models" width="1000px" height="456px">

Note: So we started looking at various models. If you haven't looked recently there are lot out there, some are extremely complicated and would not map to a checklist, and we wanted to make this a simple tool for researchers to use. We ultimately found Kristin Briney's model, from her Data Management for Researchers, to be the most useful model.
---
@title[Research Lifecycle]
### Lifecycle: Sections
- Before you begin
...
- When it’s all done
- All along the way
Note: And, while Briney uses pretty user friendly labels, we wanted to make it even friendlier. We tried to break up the lifecycle into simple sections that excluded jargon and would map across domains.
---
...
Note: The items on the checklist address roles, documentation, file management, preservation, and ways to share data.
---
@title[Resources]
### Resources
- Their own DMP
...
Note: Here are a couple examples of items. There are about 35 items in total from the 8 sections.
---
@title[Sharing]
...
Note: We created our working draft in google docs, but then decided it was the ideal way to share early versions. Of the several advantages of this approach--low technical overhead being a big one--Since we knew our version couldn't cover the specifics needs of all users, is that researchers can add a copy to their own Drives and edit as needed.
---
